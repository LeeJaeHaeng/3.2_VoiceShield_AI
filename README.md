...
- **이미지 위조 탐지 (Image Forensics)**
  ...
  - **기능**: 딥페이크로 의심되는 인물 사진이나 신분증 이미지를 분석합니다. 압축률 차이를 이용한 ELA 기법으로 육안으로 보이지 않는 조작 흔적을 찾아냅니다.
...
- **라이브 모니터링 (Live Monitoring)**
  - **기능**: 통화 중 실시간으로 오디오를 캡처하여 분석합니다. 보이스피싱 위험 단어(예: "계좌 이체", "비밀번호", "검찰")나 딥페이크 징후가 포착되면 화면의 **위험도 게이지**가 상승하며 사용자에게 경고를 보냅니다.
